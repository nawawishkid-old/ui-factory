...
abstract class Component
{
	...
	public function __construct(array $props = [], $echo = true)
	{
		...
		$this->assignEmptyContentSiblingProps(
			$this->props, 
			self::$configs['PROP_CONTENT_SUFFIX'], 
			self::$configs['PROP_CONTENT_PREPEND_SUFFIX'], 
			self::$configs['PROP_CONTENT_APPEND_SUFFIX']
		);
	...
	{
		$this->injectContentSibling(
			$name, 
			$this->getClientContent($content), 
			self::$configs['PROP_CONTENT_PREPEND_SUFFIX'], 
			false
		);
	...
	protected function injectContentChild(
		string $nosuffix_prop_name, 
		string $content, 
		string $suffix, 
		$append = true
	)
	{
		$prop =& $this->props[$nosuffix_prop_name . $suffix];
		$prop = $this->concatString($prop, $content, $append);
	}

	protected function injectContentSibling(
		string $nosuffix_prop_name, 
		string $content, 
		string $suffix, 
		$append = true
	...
	protected function concatString(string $old_string, string $new_string, $append = true)
	{
	...
	protected function assignEmptyContentSiblingProps(
		array $props, 
		string $content_suffix, 
		string $before_content_suffix, 
		string $after_content_suffix
	)
	{
		$content_suffix_length = mb_strlen($content_suffix);

		foreach ($props as $key => $value) {
			if (mb_substr($key, -$content_suffix_length, $content_suffix_length) === $content_suffix) {
				$name = $this->extractPropContentName($key);

				$this->props[$name . $before_content_suffix] = '';
				$this->props[$name . $after_content_suffix] = '';
			}
		}
	}
